...
public class BluetoothLeService extends Service {
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                if (D) Log.i(TAG, "Attempting to start service discovery:" + result);
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                if (D) Log.e(TAG, "onServicesDiscovered received: " + status);
            }
        }
    ...
    private void broadcastUpdate(final String action, final BluetoothGattCharacteristic characteristic) {
        ...
        if (UUID_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {
            ...
            if ((flag & 0x01) != 0) {
                ...
                if (D) Log.d(TAG, "Heart rate format UINT16.");
            } else {
                format = BluetoothGattCharacteristic.FORMAT_UINT8;
                if (D) Log.d(TAG, "Heart rate format UINT8.");
            }
            final int heartRate = characteristic.getIntValue(format, 1);
            if (D) Log.d(TAG, String.format("Received heart rate: %d", heartRate));
    ...
    public boolean initialize() {
        ...
        if (mBluetoothManager == null) {
            ...
            if (mBluetoothManager == null) {
                if (D) Log.e(TAG, "Unable to initialize BluetoothManager.");
                return false;
            }
        }
        ...
        if (mBluetoothAdapter == null) {
            if (D) Log.e(TAG, "Unable to obtain a BluetoothAdapter.");
            return false;
        }
    ...
    public boolean connect(final String address) {
        if (mBluetoothAdapter == null || address == null) {
            if (D) Log.w(TAG, "BluetoothAdapter not initialized or unspecified address.");
            return false;
        }
        ...
        if (device == null) {
            if (D) Log.w(TAG, "Device not found.  Unable to connect.");
            return false;
        }
    ...
    public void disconnect() {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            if (D) Log.w(TAG, "BluetoothAdapter not initialized");
            return;
        }
